...
function DrawPage (win, report, index) 
{
      try
        {
            var page = report.ReportPages[index] ;

            var canvas = win.document.getElementById ( 'page-' + index ) ;
            ...
            */
            ...
            for ( var i=0; i < page.PageElements.length; i++ )
            {
                ...
                if ( page.PageElements[i].barcode )
                {                
                  var BC_Canvas = win.document.createElement ( 'canvas' ) ;
                  context.save();     
                  var element = page.PageElements[i].barcode;

                  //the BC_Canvas is sized as part of this call
                  JsBarcode( BC_Canvas, element.data, { 
                  ...
                  try {
                      ...
                      context.drawImage ( BC_Canvas, x, y);
                  }
                  catch(oe) { alert("Error drawing barcode: " + oe.message); }
                  context.restore();
                  continue ;
                }                
            }
...
function Show (report, bPrintPreview, winName)
{
		var win = window.open ( "", winName, "width=" + report.winWidth + ",height=" + report.winHeight );
		...
		*/
		...
		if ( ( navigator.userAgent.includes ) &&  ( navigator.userAgent.includes ( "Firefox/" ) ) )
		{
		...
		*/
		...
		doc.write( "canvas{zoom:" + zoom + ";margin-top:" + report.canvasMarginTop + report.uom + ";margin-left:" + report.canvasMarginLeft + report.uom + ";margin-bottom:0;margin-right:0;}" ) ;
		...
		*/
		...
		doc.write( "<img id='printImage' class='print-hidden' onclick='window.print()' src='data:image/png;base64," +
